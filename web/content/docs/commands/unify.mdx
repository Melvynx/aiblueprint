...
| `projects` | Current working directory | Alias for repository mode. Imports project-local skills, agents, rules, and memories into `.agents`, creates only the needed category folders, links `.claude/rules` back to `.agents/rules`, creates `AGENTS.md`, and symlinks `CLAUDE.md` to it. |
...
configuration in `.claude`, `.cursor`, or `.agents`.
...
also does not create `.codex`; Codex reads the shared `.agents` project context.
